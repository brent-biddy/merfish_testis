...
# Staging is the input contract: the workflow drops every sample's zarr beside the
# notebook. Sorted so the section order does not depend on which task finished first.
...
# than trusting the two staged lists to be in the same order.
CENTROIDS = {}
