...
execute:
  warning: false
  # error: true is deliberately not set. The sample loop catches its own failures and draws
  # them, so what is left uncaught is setup -- nothing staged, a missing import -- which
  # should fail the render and its exit code rather than publish an empty deck.
---
...
# The reference template is 16:9 at 10 x 5.625in; leave room for a slide title. Every
# figure takes the width from here even when its height is its own, so a template change
...
# The obs columns this document reads, and the step that writes each. A store from an older
# pipeline otherwise fails partway into a slide with a KeyError naming the column but not
# where it comes from, once per sample. The corr_<cell type> columns are not listed:
# annotate_celltypes writes them alongside cell_type_per_cell, one per reference type, so
# their names are not known until the store is open.
REQUIRED_OBS = {
...
# A QC metric with no row above would be checked by nothing and fail inside a slide.
...
for path in ZARRS:
    ...
    # Its own try, and the only section headed by a filename: a store that will not open
    # has no sample id to head the section with. Uncaught, it would abort the cell and
    # take every later sample with it.
    try:
        ...
        # The two embeddings the slides draw, for the same reason: read_lazy leaves them
        # lazy and sc.pl.umap indexes them with a tuple of dimensions, which a lazy array
        # cannot take. Only these two -- obsm also holds the blank-probe counts as a
        # frame, which np.asarray flattens to nothing.
