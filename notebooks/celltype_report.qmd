...
author: "Brent Biddy"
date: today
# Pinned by name. Apptainer bind-mounts $HOME, so kernelspecs from
# ~/.local/share/jupyter/kernels shadow the container's own and "python3" resolves to a
# host env without the pipeline's packages.
...
# Staging is the input contract: the workflow drops every samplesheet path into its own
# input directory beside the notebook, one per file, since two samples publish their
# stores under the same name. Globbing them is how this notebook declares what it needs --
# the render step names no columns and knows nothing about zarrs.
ZARRS = sorted(Path(".").glob("input*/*.zarr"))
if not ZARRS:
...
# <sample>.centroids_<column>.h5ad, which the same pattern takes.
CENTROIDS = {}
for path in sorted(Path(".").glob("input*/*.centroids*.h5ad")):
